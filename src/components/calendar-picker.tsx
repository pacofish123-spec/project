...
interface CalendarPickerProps { startDate: string; endDate: string; onChange: (startDate: string, endDate: string) => void; bookedDates?: string[]; }

const pad = (value: number) => String(value).padStart(2, "0");
const keyForDate = (date: Date) => `${date.getFullYear()}-${pad(date.getMonth() + 1)}-${pad(date.getDate())}`;

export function CalendarPicker({ startDate, endDate, onChange, bookedDates = [] }: CalendarPickerProps) {
  const { language, t } = useLanguage();
  const booked = useMemo(() => new Set(bookedDates), [bookedDates]);

  function rangeHasBookedDay(start: string, end: string) {
    for (const key of booked) { if (key > start && key < end) return true; }
    return false;
  }
