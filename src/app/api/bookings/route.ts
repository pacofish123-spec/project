...
export async function GET() {
  try {
    ...
    const { data, error } = await supabase.from("bookings").select("*, vehicles(make, model, year, location_city, host_type)").eq("renter_user_id", user.id).order("starts_at", { ascending: false });
